...
import { DEFAULT_MAX_FILES, ISO_UTC_RE } from "./lib/constants.mjs";
...
function parseArgs(argv) {
  ...
  if (options.generatedAt && (!ISO_UTC_RE.test(options.generatedAt) || Number.isNaN(Date.parse(options.generatedAt)))) {
    throw new Error("--generated-at must be an ISO-8601 UTC date-time ending in Z");
  }
  return options;
}
