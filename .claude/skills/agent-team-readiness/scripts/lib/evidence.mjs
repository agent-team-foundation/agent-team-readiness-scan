...
const COMMAND_START = /^(?:sudo\s+)?(?:npm|pnpm|yarn|bun|make|just|cargo|go|python(?:3)?|pytest|uv|poetry|tox|nox|mvn|gradle|\.\/gradlew|dotnet|docker\s+compose|task)\b/;
const PROSE_TOKENS = new Set([
  "a", "after", "an", "always", "and", "are", "be", "been", "before", "being", "but", "could", "during",
  "if", "in", "is", "it", "its", "may", "might", "must", "never", "of", "on", "or", "our", "please",
  "should", "so", "sure", "than", "that", "the", "their", "them", "these", "they", "this", "those",
  "through", "to", "understand", "was", "we", "were", "when", "while", "will", "with", "without",
...
const ISOLATION_RE = /\b(?:git worktrees?|worktrees?|one task|one branch|branch(?:es)? per task|separate branch(?:es)?|separate workspaces?|parallel (?:work|tasks?|agents?)|concurrent (?:work|tasks?|agents?)|workspace isolation|do not share|branch naming|(?:shared|same|common|main|single) (?:branch(?:es)?|workspaces?|worktrees?|checkouts?)|share(?:d|s|ing)?\s+(?:a\s+|the\s+)?(?:branch|workspace|worktree|checkout)|(?:use|work|edit|commit|stay|remain)\b.{0,30}\bmain)\b/i;
// A policy line must direct behavior. Bare keyword mentions ("Git worktrees
// are neat.") describe, not regulate, and earn no isolation evidence.
const POLICY_DIRECTIVE_RE = /\b(?:must(?:\s+not)?|should(?:\s+not)?|shall|always|never|do\s+not|don'?t|avoid|require[sd]?|use[sd]?|using|create[sd]?|work(?:s|ing)?|keep|stays?|remains?|commits?|one\b.{0,20}\bper\b|per\s+(?:task|agent)|each\b|share[sd]?|sharing|do\s+not\s+share)\b/i;
...
function looksLikeProse(candidate) {
  return candidate
    .split(/\s+/)
    .some((token) => PROSE_TOKENS.has(token.replace(/[.,;:!?]+$/, "").toLowerCase()));
}
...
function normalizeCommand(line) {
  ...
  if (looksLikeProse(stripped)) return null;
...
function isUnsafeIsolationClause(normalized) {
  const forbidsIsolation = /\b(?:do not|don't|never|must not|cannot|can't|avoid)\b.{0,80}\b(?:(?:separate|isolated|dedicated)\s+(?:branch(?:es)?|workspaces?|worktrees?|checkouts?)|(?:one|a)\s+(?:branch|workspace|worktree|checkout)\s+per\s+(?:task|agent)|(?:branch(?:es)?|workspaces?|worktrees?|checkouts?)\s+per\s+(?:task|agent)|each\b.{0,30}\b(?:own|separate|isolated|dedicated)\s+(?:branch|workspace|worktree|checkout))\b/i.test(normalized);
  // An unqualified ban on the isolation mechanism itself ("Do not use git
  // worktrees.") is anti-isolation even without a separate/dedicated
  // qualifier — unless the clause is really banning *sharing* or scoping
  // *where* the mechanism may be used.
  const forbidsIsolationMechanism =
    /\b(?:do not|don't|never|must not|cannot|can't|avoid)\b.{0,80}\b(?:(?:git\s+)?worktrees?|separate\s+branch(?:es)?|separate\s+workspaces?)\b/i.test(normalized) &&
    !/\b(?:share[sd]?|sharing|same|common|single|another|other|someone)\b/i.test(normalized) &&
    !/\b(?:worktrees?|branch(?:es)?|workspaces?|checkouts?)\b.{0,30}\b(?:inside|within|under|outside|into|in|on|at)\b/i.test(normalized);
  const concurrent = /\b(?:parallel (?:work|tasks?|agents?)|concurrent (?:work|tasks?|agents?)|multiple agents?|all agents?|every agent|agents|agent tasks?)\b/i.test(normalized);
  ...
  const forbidsSharing = /\b(?:do not|don't|never|must not|cannot|can't|avoid)\b.{0,80}\b(?:share|shared|same|common|single|main(?:\s+branch)?|one\s+(?:branch|workspace|worktree|checkout)\s+(?:for|across|between)\s+(?:all|parallel|multiple|agents?))\b/i.test(normalized);
  const affirmativeShared = /\b(?:must|should|shall|always|required to)\b.{0,50}\b(?:use|share|edit|work|stay|remain|commit)\b/i.test(normalized);
  return forbidsIsolation || forbidsIsolationMechanism || ((concurrent || affirmativeShared) && sharedTarget && !forbidsSharing);
}
...
function hasCiExecutionStep(text) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // Only shell-executing step kinds count as verification evidence. A
    // `uses:` action reference alone (stale bots, labelers, release drafters)
    // is automation presence, not an observable check-execution gate.
    const step = line.match(/^(\s*)-?\s*(run|script|task)\s*:\s*(.*)$/i);
    if (!step) continue;
...
async function collectInstructions(root, files) {
  ...
  for (const file of files.filter(isInstructionPath).slice(0, 200)) {
    ...
    records.push({
      ...
      isolation_policies: matchingLines(text, ISOLATION_RE, 20)
        .filter(({ text: matchedText }) => POLICY_DIRECTIVE_RE.test(matchedText))
        .map(({ line, text: matchedText }) => ({
          line,
...
function makeTargetLine(text, target) {
  const lines = toLines(text);
  // `test := …`, `test ::= …`, and `test :::= …` are variable assignments, not
  // rule declarations; matching them would fabricate a runnable `make test`.
  const assignment = new RegExp(`^${target}\\s*:{0,3}=`);
...
async function collectWorkspace(root, files, instructions) {
  ...
  for (const file of contributionPolicyFiles.slice(0, 100)) {
    ...
    for (const { line, text: matchedText } of matchingLines(text, ISOLATION_RE, 20)) {
      if (!POLICY_DIRECTIVE_RE.test(matchedText)) continue;
