...
import { isValidUtcTimestamp } from "./lib/validate.mjs";
...
function validateManifest(manifest, limited) {
  ...
  if (manifest.generated_at && !isValidUtcTimestamp(manifest.generated_at)) {
    throw new Error("Calibration manifest generated_at must be a real ISO-8601 UTC date-time ending in Z");
  }
}
