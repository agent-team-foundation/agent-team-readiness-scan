...
test("equivalent shared-state wording remains unsafe without flagging per-task isolation", async () => {
  const unsafeSentences = [
    ...
    "Each agent works on the shared branch.",
    "Agents are forbidden from using separate branches.",
    "A branch per task is prohibited."
  ];
  ...
  for (const sentence of [
    ...
    "Agents should not commit directly to main; use a separate branch per task.",
    "Agents shall not share the same branch.",
    "Agents may not work on the main branch.",
    "Committing directly to main is prohibited."
  ]) {
    const result = await scanIsolationPolicy(sentence);
    try {
...
test("repository content and declared commands are never executed", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(serializedEvidence, /ghp_012345678901234567890123456789/);
    assert.doesNotMatch(serializedEvidence, /glpat-1234567890abcdefghij/);
    assert.doesNotMatch(JSON.stringify(result.report), /glpat-1234567890abcdefghij/);
    assert.match(serializedEvidence, /\[redacted-token\]/);
    assert.equal(boundedEvidenceText("api_key=ghp_012345678901234567890123456789"), "api_key=[redacted]");
  } finally {
    await result.cleanup();
  }
});
